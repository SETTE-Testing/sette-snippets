...
@SetteSnippetContainer(category = "B6", goal = "Check support for unchecked exceptions", inputFactoryContainer = B6b_UncheckedExceptions_Inputs.class)
public final class B6b_UncheckedExceptions {
    ...
    @SetteIncludeCoverage(classes = { B6b_UncheckedExceptions.class }, methods = { "calledFunction(int, int)" })
    @SetteRequiredStatementCoverage(value = 100)
    public static int call(int x, int y) {
        return calledFunction(x, y);
    }

    @SetteIncludeCoverage(classes = { B6b_UncheckedExceptions.class }, methods = { "recursive_(int, int)" })
    @SetteRequiredStatementCoverage(value = 100)
